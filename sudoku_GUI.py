...
class SudokuTile:
    ...
    This class represents a single individual tile of the Sudoku board.
        It will both store information om the tile as well as use pygame to
        display the tile.
    The constructor will takes in four parameters. The x and y parameters
...
class Board():
    '''
    This class will will use tiles created from the SudokuTile class to create
    ...
    The constructor will initialize an array which will later will contain all
        ...
        containing the information to create the boxes on the bottom of the
    ...
    def draw_window(self, solve, random, is_valid):
        ...
        Parameters:
            solve: An array where the first value is a tuple with 3 integers
                representing an rgb value and the second is a rectangle.
...
def main():
    ...
    while keep_running:
        ...
        for event in pygame.event.get():
            ...
            # checks if the cursor is hovering over the the random box
            if random_button.collidepoint(cur_position):
                sudoku_board.random_box[1] = DARK_BLUE
            else:
                sudoku_board.random_box[1] = BLUE
            # checks if the cursor is hovering over the the solve box
